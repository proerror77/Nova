...
ALTER TABLE users ADD COLUMN IF NOT EXISTS two_fa_enabled_at TIMESTAMP WITH TIME ZONE NULL;
...
-- 创建索引优化查询
CREATE INDEX IF NOT EXISTS idx_backup_codes_user_id ON two_fa_backup_codes(user_id);
CREATE INDEX IF NOT EXISTS idx_backup_codes_hash ON two_fa_backup_codes(code_hash);
-- 优化: 查询未使用的备用码
CREATE INDEX IF NOT EXISTS idx_backup_codes_unused ON two_fa_backup_codes(user_id, is_used) WHERE is_used = FALSE;
