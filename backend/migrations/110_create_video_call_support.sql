...
CREATE TABLE IF NOT EXISTS call_sessions (
    ...
    -- Constraints
    CONSTRAINT valid_status CHECK (status IN ('ringing', 'connected', 'ended', 'failed')),
    CONSTRAINT min_participants_check CHECK (max_participants >= 1),
    CONSTRAINT valid_duration CHECK (duration_ms IS NULL OR (duration_ms > 0 AND duration_ms <= 86400000))
);
...
CREATE TABLE IF NOT EXISTS call_participants (
    ...
    -- Constraints
    CONSTRAINT valid_connection_state CHECK (connection_state IN ('new', 'connecting', 'connected', 'disconnected', 'failed', 'closed'))
);
...
-- Column comments for call_sessions
...
-- Column comments for call_participants
...
-- Constraint comments
COMMENT ON CONSTRAINT min_participants_check ON call_sessions IS 'Ensures max_participants is at least 1';
COMMENT ON CONSTRAINT valid_status ON call_sessions IS 'Ensures status is one of: ringing, connected, ended, failed';
COMMENT ON CONSTRAINT valid_connection_state ON call_participants IS 'Ensures connection_state follows valid WebRTC states';
COMMENT ON CONSTRAINT valid_duration ON call_sessions IS 'Ensures duration is positive and less than 24 hours (86400000ms)';
