//! HTTP handlers for Live Streaming
//!
//! These handlers provide REST API endpoints for streaming functionality:
...
fn map_anyhow(err: Error) -> actix_web::Error {
    AppError::Internal(err.to_string()).into()
}

fn map_app_error(err: AppError) -> actix_web::Error {
    err.into()
}
