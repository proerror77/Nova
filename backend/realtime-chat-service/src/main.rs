    ...
    let matrix_client = if cfg.matrix.enabled {
        match realtime_chat_service::services::matrix_client::MatrixClient::new(cfg.matrix.clone()).await {
            Ok(client) => {
                ...
                let recovery_key = cfg.matrix.recovery_key.as_ref().filter(|k| !k.is_empty());
                if let Some(recovery_key) = recovery_key {
                    ...
                    tracing::info!("Matrix E2EE key backup not configured (MATRIX_RECOVERY_KEY not set or empty)");
