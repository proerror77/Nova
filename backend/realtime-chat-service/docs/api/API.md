...
Nova 目前採用 **Matrix-first**（Synapse + Matrix Rust SDK）。即時收訊與 E2EE 由 Matrix sync 負責。

此處的 Nova chat WebSocket 端點為舊方案，已弃用且預設不再對外暴露（`/ws/chat` 已移除）。如需即時收訊/typing/已讀回執，請改用 Matrix homeserver 的 sync 與房間事件。
...
Nova chat 已改為 **Matrix-first**。SDK 請直接使用 Matrix homeserver（Synapse）與 Matrix client SDK（例如 iOS 的 Matrix Rust SDK）來 join room / sync / send events；此服務的 WebSocket 範例已移除。
