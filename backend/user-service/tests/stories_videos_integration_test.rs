...
use serde_json::json;
use user_service::{
    config::video_config::{DeepLearningConfig, VideoConfig},
    handlers,
    services::{
        deep_learning_inference::DeepLearningInferenceService, video_service::VideoService,
    },
};
use uuid::Uuid;
        ...
        App::new()
            ...
            .service(
                web::resource("/stories")
                    .route(web::post().to(
                        move |payload: web::Json<handlers::stories::CreateStoryRequest>,
                              pool: web::Data<sqlx::PgPool>| async move {
                            handlers::create_story(
                                user_service::middleware::UserId(owner.id),
                                pool,
                                payload,
                            )
                            .await
                    ...
                    .route(
                        web::get().to(move |pool: web::Data<sqlx::PgPool>| async move {
                            handlers::list_stories(
                                user_service::middleware::UserId(follower.id),
                                pool,
                                web::Query(handlers::stories::FeedQuery { limit: 20 }),
                            )
                            .await
                        }),
                    ),
            )
            .service(web::resource("/stories_as_stranger").route(web::get().to(
                move |pool: web::Data<sqlx::PgPool>| async move {
                    handlers::list_stories(
                        user_service::middleware::UserId(stranger.id),
                        pool,
                        web::Query(handlers::stories::FeedQuery { limit: 20 }),
                    )
                    .await
            ...
            .route(
                ...
                web::post().to(
                    move |req: HttpRequest,
                          body: web::Json<user_service::models::video::CreateVideoRequest>,
                          pool: web::Data<sqlx::PgPool>,
                          vs: web::Data<VideoService>,
                          dl: web::Data<DeepLearningInferenceService>| {
                        let owner_id = owner_id;
                        async move {
                            handlers::create_video(
                                req,
                                user_service::middleware::UserId(owner_id),
                                pool,
                                body,
                                vs,
                                dl,
