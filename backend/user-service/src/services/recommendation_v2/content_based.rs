...
impl ContentBasedModel {
    ...
    pub async fn build_user_profile(&self, user_id: Uuid) -> Result<Vec<f32>> {
        // TODO: Query user interactions from ClickHouse
        // let interactions = query_user_interactions(user_id).await?;
        ...
        for (post_id, action, weight) in interactions {
