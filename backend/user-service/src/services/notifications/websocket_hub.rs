...
impl WebSocketHub {
    ...
    pub async fn broadcast(&self, message: Message) -> usize {
        ...
        for connection in connections.values() {
            if connection.state == ConnectionState::Connected && connection.sender.send(message.clone()).is_ok() {
                success_count += 1;
            }
        }
...
mod tests {
    ...
    async fn test_websocket_connection_state_transitions() {
        ...
        let updated = hub.update_connection_state(connection_id, ConnectionState::Reconnecting).await;
        assert!(updated);

        // Test transition to disconnected
        let updated = hub.update_connection_state(connection_id, ConnectionState::Disconnected).await;
        assert!(updated);

        // Test updating non-existent connection
        let fake_id = Uuid::new_v4();
        let updated = hub.update_connection_state(fake_id, ConnectionState::Connected).await;
        assert!(!updated);
    }
    ...
    async fn test_websocket_disconnect_cleanup_no_messages() {
        ...
        hub.update_connection_state(connection_id, ConnectionState::Disconnected).await;
