...
pub async fn get_s3_client(config: &S3Config) -> Result<Client, AppError> {
    ...
    client
        ...
        .map_err(|e| {
            ...
            if error_msg.contains("403") || error_msg.contains("Forbidden") {
                AppError::Internal(format!("S3 auth failed (403): Check AWS credentials"))
            } else if error_msg.contains("NoSuchBucket") {
                AppError::Internal(format!("S3 bucket not found: {}", config.bucket_name))
            } else {
...
pub fn generate_cloudfront_url(config: &S3Config, s3_key: &str) -> Result<String, AppError> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_presigned_url_generation_format() {
        // This test verifies the presigned URL format without hitting AWS
        let config = create_test_config();

        // Note: This will fail without real AWS credentials in CI
        // In production, use mock S3 client or integration tests
        let client_result = get_s3_client(&config).await;
        assert!(client_result.is_ok());

        // URL generation would require AWS connectivity
        // Test structure only
    }
    ...
    async fn test_s3_client_creation() {
        let config = create_test_config();
        let result = get_s3_client(&config).await;
    ...
    #[tokio::test]
    async fn test_s3_key_validation() {
        // Test S3 key format validation
        let valid_keys = vec![
            "posts/user123/image.jpg",
            "uploads/2024/10/photo.png",
            "thumbnails/abc-def-123.webp",
        ];
    ...
    #[tokio::test]
    async fn test_upload_image_invalid_file_path() {
        let config = create_test_config();
        let client = get_s3_client(&config).await.unwrap();

        // Try to upload non-existent file
        let result = upload_image_to_s3(
            &client,
            &config,
            "/tmp/nonexistent-file-12345.jpg",
            "posts/test-post/thumbnail.jpg",
            "image/jpeg",
        )
        .await;
        ...
        assert!(error.to_string().contains("Local file not found"));
    }

    #[tokio::test]
    async fn test_upload_image_invalid_s3_key_format() {
        // Create a temporary file for testing
        let temp_dir = TempDir::new().unwrap();
        let temp_file = temp_dir.path().join("test.jpg");
        fs::write(&temp_file, b"fake image data").unwrap();

        let config = create_test_config();
        let client = get_s3_client(&config).await.unwrap();

        // Try to upload with invalid S3 key (only 2 parts instead of 3)
        let result = upload_image_to_s3(
            &client,
            &config,
            temp_file.to_str().unwrap(),
            "invalid/key.jpg", // Missing post_id level
            "image/jpeg",
        )
        .await;
        ...
        let error = result.unwrap_err();
        assert!(matches!(error, AppError::Internal(_)));
        assert!(error.to_string().contains("Invalid S3 key format"));
    }

    #[tokio::test]
    async fn test_upload_image_metadata_extraction() {
        // Create a temporary file for testing
        let temp_dir = TempDir::new().unwrap();
        let temp_file = temp_dir.path().join("test.jpg");
        fs::write(&temp_file, b"fake image data").unwrap();
        ...
        // This will fail at S3 API call, but we can verify metadata extraction
        // by checking the error happens after validation
        let result = upload_image_to_s3(
            &client,
            &config,
            temp_file.to_str().unwrap(),
            "posts/abc-123/thumbnail.jpg",
            "image/jpeg",
        )
        .await;

        // Should fail at S3 upload (no mock), not at validation
        // If it fails with "Invalid S3 key format", metadata extraction failed
        if let Err(error) = result {
            let error_msg = error.to_string();
            assert!(!error_msg.contains("Invalid S3 key format"));
            // Expected to fail with S3 connection/auth error
            assert!(
                error_msg.contains("S3 upload failed")
                    || error_msg.contains("S3 auth failed")
    ...
    #[tokio::test]
    async fn test_delete_s3_object_structure() {
        let config = create_test_config();
        let client = get_s3_client(&config).await.unwrap();

        // This will fail at S3 API call without mock
        let result = delete_s3_object(&client, &config, "posts/abc-123/thumbnail.jpg").await;

        // Should fail at S3 operation, not parameter validation
        if let Err(error) = result {
            assert!(matches!(error, AppError::Internal(_)));
            let error_msg = error.to_string();
            assert!(
                error_msg.contains("S3 delete failed")
                    || error_msg.contains("S3 auth failed")
                    || error_msg.contains("dispatch failure")
            );
        }
    ...
    #[tokio::test]
    async fn test_cloudfront_url_various_keys() {
        let config = create_test_config();
        ...
        let test_cases = vec![
            ...
            (
                "posts/xyz-789/medium.jpg",
                "https://d1234567890.cloudfront.net/posts/xyz-789/medium.jpg",
            ),
            (
                "posts/test-post-id/original.jpg",
                "https://d1234567890.cloudfront.net/posts/test-post-id/original.jpg",
            ),
        ];

        for (s3_key, expected_url) in test_cases {
            let result = generate_cloudfront_url(&config, s3_key);
            assert!(result.is_ok());
