...
/// Saved Posts Feed endpoint - returns user's saved/bookmarked posts with full details.
/// Requires authentication. Calls social-service to get saved post IDs, then enriches with content.
#[get("/saved")]
pub async fn get_saved_feed(
    ...
    use grpc_clients::nova::social_service::v2::GetSavedPostsRequest;
    ...
    let limit = query.limit.min(100).max(1) as i32;
    let cursor = query.cursor.clone().unwrap_or_default();

    info!(
        "Getting saved feed for user: user={} limit={} cursor={}",
        user_id, limit, cursor
    );

    // Call social-service to get saved post IDs
    let mut social_client = state.grpc_pool.social();
    let saved_resp = social_client
        .get_saved_posts(GetSavedPostsRequest {
            user_id: user_id.to_string(),
            limit,
            cursor,
        })
    ...
    let post_ids: Vec<Uuid> = saved_resp
        .post_ids
        .into_iter()
        .filter_map(|id| Uuid::parse_str(&id).ok())
        .collect();
    ...
    let has_more = saved_resp.has_more;
    let total_count = saved_resp.total_count as usize;
    let next_cursor = if has_more && !saved_resp.next_cursor.is_empty() {
        Some(saved_resp.next_cursor)
    } else {
        None
    };

    info!(
        "Saved feed generated: posts={}, has_more={}, total={}",
        posts_count, has_more, total_count
    );
...
/// Liked Posts Feed endpoint - returns posts the user has liked with full details.
/// Requires authentication. Calls social-service to get liked post IDs, then enriches with content.
#[get("/liked")]
pub async fn get_liked_feed(
    ...
    use grpc_clients::nova::social_service::v2::GetLikedPostsRequest;
    ...
    info!(
        "Getting liked feed for user: user={} limit={} cursor={}",
        user_id, limit, cursor
    );

    // Call social-service to get liked post IDs
    let mut social_client = state.grpc_pool.social();
    let liked_resp = social_client
        .get_liked_posts(GetLikedPostsRequest {
            user_id: user_id.to_string(),
            limit,
            cursor,
        })
    ...
    let post_ids: Vec<Uuid> = liked_resp
        .post_ids
        .into_iter()
        .filter_map(|id| Uuid::parse_str(&id).ok())
        .collect();
    ...
    let posts_count = post_ids.len();
    let has_more = liked_resp.has_more;
    let total_count = liked_resp.total_count as usize;
    let next_cursor = if has_more && !liked_resp.next_cursor.is_empty() {
        Some(liked_resp.next_cursor)
    } else {
        None
    };

    info!(
        "Liked feed generated: posts={}, has_more={}, total={}",
        posts_count, has_more, total_count
    );
