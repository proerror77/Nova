use crate::metrics::{
    record_outbox_dlq_message, record_outbox_failure, record_outbox_latency,
    record_outbox_processed, record_outbox_retry_attempt,
};
...
impl Default for OutboxConsumerConfig {
    fn default() -> Self {
        let dlq_topic = env::var("AUTH_OUTBOX_DLQ_TOPIC")
            .or_else(|_| env::var("KAFKA_DLQ_TOPIC"))
            .map(|value| value.trim().to_owned())
            .ok()
            .filter(|value| !value.is_empty())
            .or_else(|| Some("auth-events.dlq".to_string()));
...
pub fn spawn_outbox_consumer(
    ...
    info!(
        ...
        dlq_topic = %dlq_label,
        "Starting auth-service outbox consumer"
    );
...
async fn process_batch(
    ...
    for event in events {
        ...
        if producer.is_some() {
            record_outbox_retry_attempt(&event.event_type, attempt_number as u32);
        }
        ...
        record_outbox_latency(&event.event_type, latency_seconds);

        match processing_result {
            Ok(ProcessOutcome::Published) => {
                record_outbox_processed(&event.event_type);
            ...
            Err(failure) => {
                record_outbox_failure(&event.event_type, failure.reason());
                warn!(
                ...
                if should_move_to_dlq {
                    if let (Some(producer), Some(topic)) = (producer, config.dlq_topic.as_deref()) {
                        match send_to_dlq(producer, topic, &event, &failure).await {
                            Ok(_) => {
                                record_outbox_dlq_message(&event.event_type);
                                info!(
                            ...
                            Err(dlq_err) => {
                                record_outbox_failure(&event.event_type, "dlq_error");
                                error!(
                                    error = %dlq_err,
