...
impl EmailService {
    /// Build email service from configuration. If SMTP host is empty, a no-op transport is used.
    ...
    /// Indicates whether SMTP transport is enabled.
    pub fn is_enabled(&self) -> bool {
        self.transport.is_some()
    }

    /// Send verification email with activation link.
    pub async fn send_verification_email(&self, recipient: &str, token: &str) -> AuthResult<()> {
    ...
    pub async fn send_password_reset_email(&self, recipient: &str, token: &str) -> AuthResult<()> {
    ...
    /// Generate backup verification code for manual verification flows.
    ...
    async fn send_mail(&self, recipient: &str, subject: &str, body: &str) -> AuthResult<()> {
