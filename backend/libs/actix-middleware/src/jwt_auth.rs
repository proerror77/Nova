            ...
            let claims = if let Some(redis_conn) = &redis {
                ...
                match conn.get::<_, String>(&cache_key).await {
                            ...
                            Ok(cached_claims) => {
                                ...
                                if is_token_revoked(redis_conn, token).await? {
                                    ...
                                    match validate_token_directly(token).await {
                                        ...
                                        Err(e) => {
                                            tracing::warn!(
                                                "Cached JWT failed re-validation: {}",
                                                e
                                            );
                    ...
                    Err(_) => {
                        ...
                        JWT_CACHE_MISS.inc();
                        validate_and_cache_token(token, redis_conn, &cache_key, cache_ttl_secs)
                            .await?
                    }
                }
            } else {
...
async fn validate_and_cache_token(
    ...
    tokio::spawn(async move {
        ...
        let _result: Result<(), redis::RedisError> = conn
            .set_ex(&cache_key, cached_json, cache_ttl_secs as u64)
            .await;
