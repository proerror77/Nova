    ...
    pub async fn update_preferences(
        ...
        let query = r#"
            ...
            SET
                ...
                updated_at = NOW()
            WHERE user_id = $1
            RETURNING id, user_id, enabled, like_enabled, comment_enabled,
                      follow_enabled, mention_enabled, message_enabled, stream_enabled,
                      quiet_hours_start, quiet_hours_end, prefer_fcm, prefer_apns,
                      prefer_email, updated_at
        "#;

        let row = sqlx::query(query)
            .bind(preferences.user_id)
            .bind(preferences.enabled)
            .bind(preferences.like_enabled)
            .bind(preferences.comment_enabled)
            .bind(preferences.follow_enabled)
            .bind(preferences.mention_enabled)
            .bind(preferences.message_enabled)
            .bind(preferences.stream_enabled)
            .bind(preferences.quiet_hours_start.clone())
            .bind(preferences.quiet_hours_end.clone())
            .bind(preferences.prefer_fcm)
            .bind(preferences.prefer_apns)
            .bind(preferences.prefer_email)
            .fetch_one(&self.db)
            .await
