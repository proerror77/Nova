...
use super::{FCMClient, APNsClient, APNsPriority, KafkaNotification};
use sqlx::{PgPool, Row};
use uuid::Uuid;
use chrono::Utc;
use std::sync::Arc;
...
impl NotificationService {
    ...
    pub async fn send_push_notification(
        ...
        let preferences = self.get_notification_preferences(notification.user_id).await?;
        ...
        for device in devices {
            let result = self.send_to_device(
                &device,
                &notification.title,
                &notification.body,
    ...
    async fn send_to_device(
        ...
        match device.device_type {
            DeviceType::IOS => {
                self.send_via_apns(device, title, body).await
            }
    ...
    async fn send_via_apns(
        ...
        match &self.apns_client {
            Some(apns) => {
                match apns.send(&device.device_token, title, body, APNsPriority::High).await {
    ...
    async fn send_via_fcm(
        ...
        match &self.fcm_client {
            Some(fcm) => {
                match fcm.send(&device.device_token, title, body, data).await {
                    Ok(result) => PushNotificationResult {
                        device_id: device.device_id,
                        success: result.success,
                        message_id: Some(result.message_id),
...
mod tests {
    ...
    fn test_device_type_from_string() {
        ...
        assert_eq!(DeviceType::from_string("iOS"), Some(DeviceType::IOS));
        assert_eq!(DeviceType::from_string("android"), Some(DeviceType::Android));
