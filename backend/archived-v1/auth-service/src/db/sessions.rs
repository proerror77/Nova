...
/// Get session by ID
pub async fn get_session(pool: &PgPool, session_id: Uuid) -> AuthResult<Option<Session>> {
    let session = sqlx::query_as::<_, Session>(
        r#"
...
/// Revoke a session
pub async fn revoke_session(pool: &PgPool, session_id: Uuid) -> AuthResult<()> {
    let now = Utc::now();
...
pub async fn list_sessions(pool: &PgPool, user_id: Uuid) -> AuthResult<Vec<Session>> {
...
pub async fn update_last_activity(pool: &PgPool, session_id: Uuid) -> AuthResult<()> {
    let now = Utc::now();
