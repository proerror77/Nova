...
final class VoiceMessageService: Sendable {
    ...
    private func requestPresignedURL(
        ...
        // Call backend endpoint: POST /conversations/:id/messages/audio/presigned-url
        let request = AudioPresignedUrlRequest(
            fileName: fileName,
            contentType: mimeType
        )

        let endpoint = "conversations/\(conversationId)/messages/audio/presigned-url"

        let response: AudioPresignedUrlApiResponse = try await httpClient.request(
            method: .post,
            endpoint: endpoint,
            body: request
        )
    ...
    private func sendAudioMessageMetadata(
        ...
        let endpoint = "conversations/\(conversationId)/messages/audio"

        let response: AudioMessageDto = try await httpClient.request(
            method: .post,
            endpoint: endpoint,
            body: request
        )
